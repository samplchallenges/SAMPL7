...
def bootstrap_exptnoise(calc1, expt1, exptunc1, returnunc=False):
    """
    Take two datasets (equal length) of calculated and experimental values.
    Construct new datasets of equal length by picking, with replacement, a set of indices to use from both sets.
    Return the two new datasets. To take into account experimental uncertainties, random noise is added
    to the experimental set, distributed according to gaussians with variance taken from the experimental uncertainties.
    Approach suggested by J. Chodera.
    Optionally, 'returnunc = True', which returns a third value -- experimental uncertainties corresponding to the
...
class logPSubmission(SamplSubmission):
    ...
    # Section of the submission file.
    #SECTIONS = {'Predictions', 'Participant name', 'Software', 'Category', 'Method'}
    ...
    def compute_logP_statistics(self, experimental_data, stats_funcs):
        ...
        bootstrap_statistics = compute_bootstrap_statistics(data.to_numpy(), stats_funcs, n_bootstrap_samples=10000) #10000
...
def load_all_submissions(directory_path, user_map, method_map):
    """
    ...
    print("All submissions (ranked, nonranked, REF, and NULL):", receipt_ids)
...
class logPSubmissionCollection:
    ...
    def __init__(self, submissions, experimental_data, output_directory_path, logP_submission_collection_file_path, ignore_refcalcs = True, ranked_only = True, allow_multiple = False):
        ...
        for submission in submissions:
            ...
            for mol_ID, series in submission.data.iterrows():
                ...
                data.append({
                    ...
                    'logPapp (exp)': logP_mean_exp,
                    'logPapp SEM (exp)': logP_SEM_exp,
    ...
    def generate_correlation_plots_with_SEM(self):
        ...
        for receipt_id in self.data.receipt_id.unique():
            ...
            plot_correlation_with_SEM(x_lab='logPapp (exp)', y_lab='logPapp (calc)',
                                      x_err_lab='logPapp SEM (exp)', y_err_lab='logPapp SEM (calc)',
    ...
    def generate_molecules_plot(self):
        ...
        sns.violinplot(y='Molecule ID', x='$\Delta$logPapp error (calc - exp)', data=data_ordered_by_mol_ID,
    ...
    def generate_absolute_error_vs_molecule_ID_plot(self):
        ...
        self.data.loc[:, "absolute error"] = np.absolute(self.data.loc[:, "$\Delta$logPapp error (calc - exp)"])
...
def generate_statistics_tables(submissions,
                                stats_funcs,
                                directory_path,
                                file_base_name,
                                sort_stat=None,
...
def generate_performance_comparison_plots(statistics_filename, directory_path, ignore_refcalcs = False, ranked_only = True):
        ...
        '''barplot_with_CI_errorbars(df=df_statistics_MAE, x_label="ID", y_label="MAE",
                                    y_lower_label="MAE_lower_bound",
                                    y_upper_label="MAE_upper_bound", figsize=(22,10))
        #plt.title("Method category: Empirical", fontdict={'fontsize': 22})
        #plt.ylim(0.0, 7.0)
...
if __name__ == '__main__':
    ...
    with open(EXPERIMENTAL_DATA_FILE_PATH, 'r') as f:
        names = ('Molecule ID', 'logP mean', 'logP SEM', 'Assay Type', 'Isomeric SMILES')
        experimental_data = pd.read_csv(f, names=names, skiprows=1)

    # Convert numeric values to dtype float.
    for col in experimental_data.columns[1:6]:
    ...
    # ==========================================================================================
    # Analysis of ranked and non-ranked blind submissions
    # ==========================================================================================

    # Load submissions data.
    #submissions_logP = load_submissions(LOGP_SUBMISSIONS_DIR_PATH, user_map, method_map)
    all_submissions_logP = load_all_submissions(LOGP_SUBMISSIONS_DIR_PATH, user_map, method_map)
    #
    ...
    #
    collection_logP= logPSubmissionCollection(all_submissions_logP, experimental_data,
    ...
    for all_submissions, type in zip([all_submissions_logP], ['logP']):
         generate_statistics_tables(all_submissions,
                                    stats_funcs,
