...
**Submission format**: Your submissions must consist of a completed submission description following [our format](stage2_submission_template.txt) (with a file name beginning with the characters `PHIP2_2` and ending with `.txt`) and a .tar.gz `pose.tgz` file providing your pose predictions compressed and archived in a single directory. This file is used to submit one set of predicted protein-ligand poses, where up to five poses are permitted for each protein-ligand pair (if multiple poses are submitted, subsequent poses are given decreasing weight in judging). Each pose tgz file must contain, for each ligand, a minimum of one and up to 5 protein structure PDB files and 5 corresponding ligand poses (in MDL `.mol`, format, `.sdf` format, or `.mol2` format) predicted by a method described in the separately uploaded pose prediction description file.  
...
- PHIP2-F13-2.pdb
- PHIP2-F13-2.sdf
if you were submitting two poses. (The lowest numbered poses will be assumed to be those predicted to be most favorable.) These would be placed into a single directory along with any other files to be submitted (minimally, poses for [all compounds binding in S1](Stage-2-input-data/site-1_fragment_hits.csv)), then tarred, gzipped, and uploaded to our submission site.
