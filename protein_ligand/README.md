...
Please, note that predictions should be done considering the C2 crystal form and soaking method described in Stage 1. Other, structures are available on the PDB but were identified using another crystal form. Screening against different crystal forms often lead to differential fragment-hit identification due to solution and solid states effects such as pH and crystal packing. In addition, the compound stocks were purchased as as racemic mixes implying that the higher affinity conformer should be revealed in the electron density. However, the both stereoisomers can also bind the target resulting in an average  density. Thus, we are asking the participant to take this information into accound. Participand can submit poses for multiple stereoismoers which should be ordered according to their predicted priority. The evaluation of poses will done by comparing the predicted poses of each stereoisomer to their matching experimental result when multiple experital pose exist for stereoisomers. Therefore, each 5 poses submited per ligand can be composed of different stereoisomers.
